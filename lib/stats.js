...
Stats.prototype = {
  ...
  sorted: function () {
    ...
    if (this._raw) {
      ...
      // This is a performance enhancement to help when the spread
      // between min and max is large. In node, elements are iterated
      // in increasing order, IFF all elements are >= 0; If any elements are
      // < 0, the negative numbers are iterated after the positive numbers.
      if (this.min >= 0) {
        for (var i in this._histogram) {
          var frequency = this._histogram[i] || 0;
          var value = +i;
          while (frequency) {
            this._sorted.push(value);
            frequency--;
          }
        }
      } else {
        ...
        // Perhaps a better way is to see if the spread is <= this.length^2.
        // If so, convert keys to an array, sort the array, then iterate.
  ...
  ccdf: function (value) {
    ...
    if (typeof value === 'number') return 1 - this.cdf(value);

    var ccdfs = {};
    var cdfs = this.cdf();
    for (var key in cdfs) {
      ccdfs[key] = 1 - cdfs[key];
    }
